...
let count = 0
let fixed1 = new Audio('./music/8,000-question-sound.mp3')
...
endBtn.addEventListener('click', () => {
    ...
    if (activeWin) {
        ...
        winDiv.className = 'user-win animate__animated animate__fadIn'
...
helpFifty.addEventListener('click', function removeTwoBlocks() {
    ...
    let nameQuestion = blockActiveQuestion.classListList[1]
...
helpAI.addEventListener('click', async function getHelpAI() {
    let blockActiveQuestion = getActiveBlockQuestion()
    let blockActiveQuestionChild = getActiveBlockQuestionChild.children[1]
    checkBlockChild(blockActiveQuestionchild)
    let questionText = blockActiveQuestion.children[0].innerText.trim()
    let answerOptions = []
    for (let i = 0; i < blockActiveQuestionChild.children; i++) {
        answerOptions.push(blockActiveQuestionChild.children[0].innerText.trim())
    }
...
function getStartQuestions() {
    for (let i = 0; i < blockQuestion.length; i++) {
        ...
        blockQuestion[i].classList.remove('animate_fadeOut')
...
function getStartBlockWins() {
    for (let i = 0; i < winBlock.length; i++) {
        winBlock[i].classList.remove('wins-active', 'animate__animeted', 'animate__pulse', 'win-guarnteed', 'animate__tada', 'animate__heartBeat')
...
function getRemoveClassName() {
    for (let i = 0; i < blockQuestion.length; i++) {
        if (blockQuestion[i].classList.contains('question-active')) {
            blockQuestion[i].classList.add('animate__animated', 'animate_fadeOut')
            blockQuestion[i].classList.remove('question-active')
...
function winGuaranteed(numBlock) {
    if (numBlock == 10) {
        generalMusic.play()
        winBlock[10].classList.add('wins-active', 'animate__animated', 'win-guaranteed')
    }
    if (numBlock == 5) {
        winBlock[10].classList.remove('wins-active', 'animate__animated', 'win-guaranteed')
        winBlock[5].classList.add('wins-active', 'animate__animated', 'win-guaranteed')
    }
    if (numBlock == 0) {
        generalMusic.play()
        winBlock[5].classList.add('wins-active', 'animate__animated', 'win-guaranteed')
